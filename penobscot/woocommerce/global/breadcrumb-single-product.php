 ...
 */
/* mods
 * 	14Dec15 - breadcrumb to only show immediate parent category
 *  16Dec15 - disallow the link.
 */
	...
	if ( get_post_type() == 'product' ) {
		   if(!isset($flatsome_opt['breadcrumb_home']) || $flatsome_opt['breadcrumb_home']){ 
			...
			if ( $terms = wp_get_post_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				...
				/*  zig  -- $ancestors = get_ancestors( $main_term->term_id, 'product_cat' );
				...
				*/
				echo $before . $main_term->name . $after;
